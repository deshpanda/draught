...
export const api = {
  ...
  fav: (brewery, beer, on) =>
    req(`/fav/${encodeURIComponent(brewery)}/${encodeURIComponent(beer)}`,
        { method: on ? 'POST' : 'DELETE' }),
  likeReview: (id, on) => req(`/pours/${id}/like`, { method: on ? 'POST' : 'DELETE' }),
  comments: (id) => req(`/pours/${id}/comments`),
  addComment: (id, body) => req(`/pours/${id}/comments`, { method: 'POST', body: { body } }),
  deleteComment: (id) => req(`/comments/${id}`, { method: 'DELETE' }),
  tag: (t) => req(`/tags/${encodeURIComponent(t)}`),
